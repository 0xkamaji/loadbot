...
pub(crate) fn scope(control: &Control) -> Scope {
    Scope(CURRENT.with(|current| current.replace(control.clone())), std::marker::PhantomData)
}
pub(crate) fn current_control() -> Control { CURRENT.with(|current| current.borrow().clone()) }
...
impl Drop for Scope {
    fn drop(&mut self) { CURRENT.with(|current| { current.replace(self.0.clone()); }); }
}
...
impl std::fmt::Display for Cancelled {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.write_str("operation cancelled") }
}
...
pub enum Event {
    OperationStarted,
    OperationFinished { outcome: crate::interaction::OperationStatus, partial: bool },
    Starting { program: std::ffi::OsString, arguments: Vec<std::ffi::OsString>, directory: Option<std::path::PathBuf> },
    Started { pid: u32 },
...
impl Default for Control {
    fn default() -> Self { Self { cancellation: Cancellation::default(), observer: None, terminal: true } }
}
impl Control {
...
pub fn execute(command: &mut Command, mode: Mode, control: &Control) -> Result<Output> {
    ...
    if let Err(error) = &result && error.downcast_ref::<Cancelled>().is_none() {
        control.emit(Event::Failed { diagnostic: format!("{error:#}") });
    }
    result
}
...
fn execute_inner(command: &mut Command, mode: Mode, control: &Control) -> Result<Output> {
    ...
    let pid = child.id();
    let owner = match platform::Owner::new(&child, !piped || (control.terminal && matches!(mode, Mode::Capture { .. }))) {
        Ok(owner) => owner,
        Err(error) => {
            child.kill().context("process ownership failed and child could not be stopped")?;
            child.wait().context("process ownership failed and child could not be reaped")?;
            return Err(error);
        }
    };
    ...
    if piped {
        readers.push(drain(child.stdout.take().unwrap(), Stream::Stdout, sender.clone(), reader_stop.clone())?);
        readers.push(drain(child.stderr.take().unwrap(), Stream::Stderr, sender.clone(), reader_stop.clone())?);
    }
    drop(sender);
    ...
    loop {
        if status.is_none() { status = child.try_wait().context("could not inspect process status")?; }
        ...
        if cleanup_started.is_some_and(|start| start.elapsed() > Duration::from_secs(5)) && !closed {
            ...
            read_error = Some(std::io::Error::other("descendant retained output pipes after process termination; cleanup could not be verified"));
        }
        if status.is_some() && closed { break; }
        match receiver.recv_timeout(Duration::from_millis(10)) {
            Ok((stream, Ok(bytes))) => {
                if let Mode::Capture { limit } = mode {
                    let buffer = if stream == Stream::Stdout { &mut stdout } else { &mut stderr };
            ...
            Err(mpsc::RecvTimeoutError::Disconnected) => {
                closed = true;
                if status.is_none() { std::thread::sleep(Duration::from_millis(10)); }
            }
            Err(mpsc::RecvTimeoutError::Timeout) => {}
        }
    }
    for reader in readers { reader.join().map_err(|_| anyhow::anyhow!("process output reader panicked"))?; }
    let status = status.unwrap();
    owner.finish().context(CleanupIncomplete)?;
    control.emit(Event::Exited { pid, status });
    if cleanup_incomplete { return Err(CleanupIncomplete.into()); }
    if let Some(error) = read_error { return Err(error).context("could not read process output or verify cleanup"); }
    if cancelled {
        control.emit(Event::Cancelled { pid });
        return Err(Cancelled.into());
    }
    if overflow { anyhow::bail!("process output exceeded capture limit; streamed output remains available to the observer"); }
...
fn drain(mut pipe: impl Pipe, stream: Stream, sender: mpsc::SyncSender<Chunk>, stop: Arc<AtomicBool>) -> Result<std::thread::JoinHandle<()>> {
    #[cfg(unix)]
    unsafe {
        let flags = libc::fcntl(pipe.as_raw_fd(), libc::F_GETFL);
        if flags == -1 || libc::fcntl(pipe.as_raw_fd(), libc::F_SETFL, flags | libc::O_NONBLOCK) == -1 {
    ...
    Ok(std::thread::spawn(move || {
        ...
        loop {
            ...
            unsafe {
                let mut available = 0;
                if windows_sys::Win32::System::Pipes::PeekNamedPipe(pipe.as_raw_handle(), std::ptr::null_mut(), 0, std::ptr::null_mut(), &mut available, std::ptr::null_mut()) == 0 {
                ...
                if available == 0 { std::thread::sleep(Duration::from_millis(10)); continue; }
            }
            match pipe.read(&mut buffer) {
                Ok(0) => break,
                Ok(count) => if sender.send((stream, Ok(buffer[..count].to_vec()))).is_err() { break; },
                Err(error) if error.kind() == std::io::ErrorKind::Interrupted => continue,
                Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => std::thread::sleep(Duration::from_millis(10)),
                Err(error) => { let _ = sender.send((stream, Err(error))); break; }
            }
        }
    }))
}
...
mod platform {
    ...
    pub fn prepare(command: &mut Command) { command.process_group(0); }
    pub struct Owner { pid: i32, foreground: Option<i32>, finished: std::cell::Cell<bool> }
    impl Owner {
        pub fn new(child: &std::process::Child, terminal: bool) -> Result<Self> {
            let pid = child.id() as i32;
            let foreground = unsafe { libc::tcgetpgrp(0) };
            let foreground = (terminal && foreground == unsafe { libc::getpgrp() }).then_some(foreground);
            let owner = Self { pid, foreground, finished: std::cell::Cell::new(false) };
        ...
        pub fn terminate(&self) -> Result<()> {
            if unsafe { libc::kill(-self.pid, libc::SIGKILL) } != 0 {
                let error = std::io::Error::last_os_error();
                if error.raw_os_error() != Some(libc::ESRCH) { return Err(error).context("could not terminate process group"); }
            }
            Ok(())
        }
        pub fn finish(&self) -> Result<()> {
            ...
            loop {
                if !group_running(self.pid)? { self.finished.set(true); return Ok(()); }
                if std::time::Instant::now() >= deadline {
                    anyhow::bail!("process group {} cleanup is incomplete; cancellation has not completed", self.pid);
                }
    ...
    fn group_running(group: i32) -> Result<bool> {
        if unsafe { libc::kill(-group, 0) } != 0 && std::io::Error::last_os_error().raw_os_error() == Some(libc::ESRCH) {
            return Ok(false);
        }
        #[cfg(target_os = "linux")]
        {
            ...
            for entry in std::fs::read_dir("/proc")? {
                ...
                let stat = match std::fs::read_to_string(entry.path().join("stat")) {
                    ...
                    Err(error) => return Err(error).context("could not verify process group cleanup"),
                };
                let Some((_, fields)) = stat.rsplit_once(") ") else { continue; };
                let fields: Vec<_> = fields.split_whitespace().take(3).collect();
                if fields.len() == 3 && fields[2].parse::<i32>() == Ok(group) && !matches!(fields[0], "Z" | "X") {
                    return Ok(true);
                }
            }
            Ok(false)
    ...
    impl Drop for Owner {
        fn drop(&mut self) {
            if !self.finished.get() { let _ = self.terminate(); }
            if let Some(foreground) = self.foreground { let _ = set_foreground(foreground); }
        }
    }
    fn set_foreground(group: i32) -> Result<()> {
        ...
        unsafe {
            ...
            if result != 0 { return Err(std::io::Error::from_raw_os_error(result).into()); }
            let result = libc::tcsetpgrp(0, group);
            let error = std::io::Error::last_os_error();
            libc::pthread_sigmask(libc::SIG_SETMASK, &old, std::ptr::null_mut());
            if result != 0 { return Err(error.into()); }
        }
        Ok(())
    }
}
