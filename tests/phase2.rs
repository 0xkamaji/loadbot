...
fn worker(role: &str) -> Command {
    ...
    command
        .args(["--exact", "controlled_child", "--nocapture"])
...
#[test]
fn controlled_child() {
    let Ok(role) = std::env::var("LOADBOT_PHASE2_CHILD") else {
        return;
    };
    match role.as_str() {
        "lease" => {
        ...
        "config" | "shortcut" => {
            ...
            let result = if role == "config" {
                ...
                shortcuts::save(
                    Path::new(&resource),
                    &name,
                    Shortcut::new("personal".into(), "demo".into(), "run.sh".into()).unwrap(),
                )
            };
            match result {
...
fn hold(resource: &Path) -> std::process::Child {
    let mut child = worker("lease")
        .env("LOADBOT_PHASE2_RESOURCE", resource)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
...
fn concurrent_document_transactions_are_busy_then_preserve_both_updates() {
    ...
    for role in ["config", "shortcut"] {
        ...
        let edit = |name: &str| {
            worker(role)
                .env("LOADBOT_PHASE2_RESOURCE", &path)
                .env("LOADBOT_PHASE2_NAME", name)
                .status()
                .unwrap()
                .code()
                .unwrap()
...
fn repository_mutation_conflicts_and_process_exit_releases_lease() {
    ...
    let error = loadbot::operations::catalog_add(
        &paths,
        "personal",
        "local.git".into(),
        true,
...
fn failed_transaction_and_recovery_keep_existing_data() {
    ...
    let result: anyhow::Result<()> =
        config::update(&path, |_| anyhow::bail!("injected edit failure"));
    assert!(result.is_err());
    assert_eq!(fs::read(&path).unwrap(), before);
    config::update(&path, |config| {
        config.extra.insert("kept".into(), true.into());
        Ok(())
    })
    .unwrap();
    ...
    assert_eq!(
        config::load(&path).unwrap().extra["kept"].as_bool(),
        Some(true)
    );
...
fn confirmed_removal_cannot_delete_a_replaced_definition() {
    ...
    assert_eq!(
        shortcuts::load(&path).unwrap().shortcuts["demo"],
        replacement
    );
}
...
fn cancelled_catalog_inspection_is_not_downgraded_to_a_skipped_catalog() {
    ...
    config::update(&paths.config(), |config| {
        config.catalogs.insert(
            "personal".into(),
            config::CatalogSource::new("local.git".into(), true),
        );
        Ok(())
    })
    .unwrap();
    ...
    assert!(
        !report
            .notices
            .iter()
            .any(|notice| matches!(notice, loadbot::interaction::Notice::SkippedCatalog { .. }))
    );
}
        ...
        observer: Some(Arc::new(move |event| {
            if let Event::Output { bytes: chunk, .. } = event {
                ...
                if String::from_utf8_lossy(&bytes)
                    .lines()
                    .any(|line| line.starts_with("DESCENDANT:"))
                {
                    request.cancel();
                }
            }
...
fn cancellation_after_definition_save_reports_partial_work_without_rollback() {
    use loadbot::{
        catalog,
        interaction::{Interaction, Notice, OperationContext, OperationStatus},
        operations,
        paths::Paths,
    };
    let temporary = tempfile::tempdir().unwrap();
    ...
    let git = |args: &[&str]| {
        let output = Command::new("git")
            ...
            .env("GIT_CONFIG_GLOBAL", temporary.path().join("absent"))
            .env("GIT_AUTHOR_NAME", "Loadbot Test")
            .env("GIT_AUTHOR_EMAIL", "loadbot@example.test")
            .env("GIT_COMMITTER_NAME", "Loadbot Test")
            .env("GIT_COMMITTER_EMAIL", "loadbot@example.test")
            .output()
            .unwrap();
        assert!(
    ...
    config::update(&paths.config(), |local| {
        local.catalogs.insert(
            "personal".into(),
            config::CatalogSource::new("local-catalog.git".into(), true),
        );
        Ok(())
    })
    .unwrap();
    ...
    impl Interaction for CancelAfterSave {
        fn notice(&mut self, notice: &Notice) {
            if matches!(notice, Notice::ToolAdded { .. }) {
                self.0.cancel();
            }
        }
    }
    ...
    assert!(
        report
            .notices
            .iter()
            .any(|notice| matches!(notice, Notice::ToolAdded { .. }))
    );
    assert!(
    ...
    assert!(
        report
            .notices
            .iter()
            .any(|notice| matches!(notice, Notice::CatalogCommitted { .. }))
    );
