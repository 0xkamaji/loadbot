...
new_case() {
    ...
    for utility in sh awk sed stat id grep cksum mkdir mktemp cat date cp chmod mv rm dirname ln wc; do
...
run_interactive() {
    answer=$1
    set +e
    printf '%s\n' "$answer" | /usr/bin/script -qefc "/bin/sh '$PROJECT/setup.sh'" /dev/null >"$OUTPUT" 2>&1
    STATUS=$?
    set -e
}
...
# Apt proposal, decline, noninteractive safety, failures, and recheck.
new_case apt
...
assert_contains "exact apt packages displayed" "sudo apt-get install -y cargo rustc" "$OUTPUT"
assert_contains "apt package command executed" "sudo apt-get install -y cargo rustc" "$COMMAND_LOG"
...
assert_contains "noninteractive prints exact required command" "sudo apt-get install -y cargo" "$OUTPUT"
...
assert_contains "recheck reports remaining command" "remain missing: cargo" "$OUTPUT"
...
new_case pacman
rm "$FAKE_BIN/apt-get" "$FAKE_BIN/cargo" "$FAKE_BIN/rustc"
make_manager pacman
FAKE_INSTALL_PREREQUISITES=1; export FAKE_INSTALL_PREREQUISITES
run_interactive y
assert_contains "pacman manager detected" "  pacman" "$OUTPUT"
assert_contains "exact pacman command displayed" "sudo pacman -S --needed cargo rust" "$OUTPUT"
assert_contains "exact pacman command executed" "sudo pacman -S --needed cargo rust" "$COMMAND_LOG"

new_case unsupported
rm "$FAKE_BIN/apt-get" "$FAKE_BIN/cargo"
run_noninteractive
